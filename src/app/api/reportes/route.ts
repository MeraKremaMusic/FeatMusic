...
export async function POST(request: Request) {
  ...
  // La base de datos es la fuente de verdad. El correo es solo un aviso
  // secundario y su fallo no debe hacer desaparecer un reporte ya recibido.
  let reporte;

  try {
    reporte = await prisma.reporteUsuario.create({
      data: {
    ...
    console.error("[FeatMusic] No se pudo guardar el reporte.", error);

    return Response.json(
      {
        ok: false,
        mensaje:
          "No pudimos guardar el reporte en este momento. Inténtalo nuevamente más tarde.",
      },
      { status: 503 },
    );
  }
  ...
  try {
    await enviarReporteUsuarioPorCorreo({
      ...
      motivo,
      descripcion,
      enviadoEn: reporte.creadoEn,
    });
  } catch (error) {
    console.error(
      `[FeatMusic] El reporte #${reporte.id} se guardó, pero falló el aviso por correo.`,
      error,
    );
  }
