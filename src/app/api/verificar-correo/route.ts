...
export async function POST(request: Request) {
  try {
    ...
    if (!pendiente) {
      return redirigir(
        rutaVerificacion(resultado.data.correo, "solicitud-no-encontrada"),
      );
    }

    if (pendiente.codigoExpiraEn < new Date()) {
      return redirigir(
        rutaVerificacion(resultado.data.correo, "codigo-vencido"),
      );
    }
    ...
    if (!codigoCorrecto) {
      ...
      return redirigir(
        rutaVerificacion(resultado.data.correo, "codigo-incorrecto"),
      );
    }
    ...
    const usuario = await prisma.$transaction(async (tx) => {
      ...
      const nuevoUsuario = await tx.usuario.create({
        data: {
          ...
          // El rol real se elige después, al completar el perfil.
