...
async function guardarPropuestaConCupo(
  ideaId: number,
  remitenteId: number,
  puedeEnviarConCuposCompletos: boolean,
  datosAudio: {
    mensaje: string | null;
    audioUrl: string;
    ...
    try {
      return await prisma.$transaction(
        async (tx) => {
          ...
          // Gratis y Creator siguen sujetos al límite del propietario.
          // PRO puede crear/reintentar aunque el contador ya haya llegado al límite.
          if (
            (modo === "NUEVA" || modo === "REINTENTO") &&
            !puedeEnviarConCuposCompletos
          ) {
            const limitePropuestas = obtenerLimitesPlan(
              idea.usuario.plan,
            ).propuestasPorIdea;
            ...
            if (cuposOcupados >= limitePropuestas) {
              throw new ErrorPropuesta(
...
export async function POST(request: Request, contexto: ContextoRuta) {
  ...
  try {
    ...
    // FEATMUSIC_PRO_CUPOS_COMPLETOS_V1
    // Sincronizamos ambos planes en servidor. El beneficio se decide aquí,
    // nunca a partir de un dato enviado por el navegador.
    const [estadoPlanPropietario, estadoPlanRemitente] = await Promise.all([
      sincronizarPlanUsuario(idea.usuarioId),
      sincronizarPlanUsuario(sesion.usuarioId),
    ]);
    const modoPrevio = resolverModoEnvio(idea.propuestas[0] ?? null);

    const limitePropuestasIdea = obtenerLimitesPlan(
      estadoPlanPropietario?.plan ?? idea.usuario.plan,
    ).propuestasPorIdea;
    const puedeEnviarConCuposCompletos = obtenerLimitesPlan(
      estadoPlanRemitente?.plan,
    ).puedeEnviarConCuposCompletos;

    if (
      (modoPrevio === "NUEVA" || modoPrevio === "REINTENTO") &&
      !puedeEnviarConCuposCompletos &&
      idea._count.propuestas >= limitePropuestasIdea
    ) {
      return respuestaError(
        `Esta idea ya tiene sus ${limitePropuestasIdea} cupos ocupados.`,
        409,
      );
    }
