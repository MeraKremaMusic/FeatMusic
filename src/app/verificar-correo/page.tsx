  ...
  return (
    <main className="flex min-h-screen items-center justify-center bg-black px-6 py-12 text-white">
      <section className="w-full max-w-md rounded-3xl border border-zinc-800 bg-zinc-950 p-8 sm:p-10">
        ...
        <p className="mt-8 text-sm font-semibold text-yellow-400">Paso 2 de 2</p>
        ...
        {(parametros.enviado || parametros.reenviado) && !error ? (
          <p className="mt-5 rounded-xl border border-yellow-500/25 bg-yellow-500/10 p-3 text-sm text-yellow-200">
            {parametros.reenviado ? "Enviamos un código nuevo." : "Revisa tu bandeja de entrada y la carpeta de spam."}
          </p>
        ) : null}

        {error ? (
          <p role="alert" className="mt-5 rounded-xl border border-yellow-900 bg-yellow-950/50 p-3 text-sm text-yellow-200">
            {error}
          </p>
        ) : null}
        ...
        <form action="/api/verificar-correo" method="post" className="mt-7 space-y-5">
          ...
          <label className="block">
            ...
            <input
              ...
              pattern="[0-9]{6}"
              placeholder="000000"
              className="w-full rounded-xl border border-zinc-800 bg-black px-4 py-4 text-center text-2xl font-semibold tracking-[0.45em] outline-none transition placeholder:tracking-[0.25em] placeholder:text-zinc-700 focus:border-yellow-500"
            />
          </label>
        ...
        <form action="/api/reenviar-codigo" method="post" className="mt-5 text-center">
          <input type="hidden" name="correo" value={correo} />
          <button type="submit" className="text-sm font-medium text-yellow-300 transition hover:text-yellow-200">
            Reenviar código
          </button>
