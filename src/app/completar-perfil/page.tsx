  ...
  return (
    <main className="min-h-screen bg-black px-5 py-10 text-white sm:px-8">
      <section className="mx-auto max-w-4xl rounded-3xl border border-zinc-800 bg-zinc-950 p-6 sm:p-10">
        <p className="text-sm font-semibold text-yellow-400">Último paso</p>
        <h1 className="mt-2 text-3xl font-bold sm:text-4xl">
          Personaliza tu perfil
        </h1>
        <p className="mt-3 text-zinc-400">
          Así podremos ayudarte a conectar con las personas y proyectos
          adecuados.
        </p>
        ...
        {error ? (
          <p
            role="alert"
            className="mt-6 rounded-xl border border-yellow-900 bg-yellow-950/50 p-4 text-sm text-yellow-200"
          >
        ...
        >
          ...
          <fieldset>
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              {roles.map(([valor, etiqueta]) => (
                <label
                  key={valor}
                  className="cursor-pointer rounded-xl border border-zinc-800 bg-black p-4 transition hover:border-yellow-500"
                >
                  <input
          ...
          <fieldset>
            <legend className="mb-3 text-sm font-medium">
              ...
              <span className="text-zinc-500">(elige hasta 5)</span>
            </legend>
            <div className="flex flex-wrap gap-2">
              {generos.map((genero) => (
                <label
                  key={genero}
                  className="cursor-pointer rounded-full border border-zinc-800 px-3 py-2 text-sm transition hover:border-yellow-500"
                >
                  <input
                    className="mr-2 accent-yellow-500"
                    type="checkbox"
          ...
          <button
            type="submit"
            className="w-full rounded-full bg-yellow-600 px-6 py-4 font-semibold transition hover:bg-yellow-500"
          >
  ...
  return (
    <label className="block">
      ...
      <input
        ...
        name={nombre}
        defaultValue={valor}
        className="w-full rounded-xl border border-zinc-800 bg-black px-4 py-3 outline-none transition focus:border-yellow-500"
      />
    </label>
  );
}
