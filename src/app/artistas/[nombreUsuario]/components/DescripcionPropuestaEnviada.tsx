...
type DescripcionPropuestaEnviadaProps = {
  tituloIdea: string;
...
export default function DescripcionPropuestaEnviada({
  tituloIdea,
  nombreDestinatario,
  nombreUsuario,
  fecha,
  ...
  estado,
  respuesta,
}: DescripcionPropuestaEnviadaProps) {
  ...
  const textoDescripcion =
    descripcion?.trim() || "No agregaste una descripción a esta propuesta.";
  const textoRespuesta = respuesta?.trim() || null;
    ...
    montado && abierta
      ? createPortal(
          <>
            <button
              type="button"
              aria-label="Cerrar descripción de la propuesta enviada"
              className="fixed inset-0 z-40 bg-slate-950/20 backdrop-blur-[5px]"
              style={{
            ...
            >
              ...
              >
                ...
                <div className="mt-4 grid gap-3">
                  ...
                  {textoRespuesta && (
                    <div className="rounded-xl border border-yellow-300 bg-yellow-50 px-3.5 py-3">
                      <p className="text-[9px] font-black uppercase tracking-[0.12em] text-black">
  ...
  return (
    <>
      <button
        type="button"
        aria-label={`Ver descripción de tu propuesta para ${tituloIdea}`}
