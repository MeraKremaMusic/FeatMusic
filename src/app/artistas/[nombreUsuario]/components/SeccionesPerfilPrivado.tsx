...
import IdeasMusicalesCard, { type IdeaPanel } from "@/app/panel/components/IdeasMusicalesCard";
...
const IDEAS_VACIAS_PUBLICAR: IdeaPanel[] = [];
...
// FEATMUSIC_MODAL_CANCELAR_ENVIO_V1
function IconoCerrarCancelar({
  className = "h-4 w-4",
}: {
  className?: string;
}) {
...
export default function SeccionesPerfilPrivado({
  ...
  const [cancelarEnvioId, setCancelarEnvioId] = useState<number | null>(null);
  ...
  function abrirModalCancelarEnvio(propuestaId: number) {
    if (procesandoId !== null) return;
    setError("");
    setCancelarEnvioId(propuestaId);
  }

  function cerrarModalCancelarEnvio() {
    if (procesandoId !== null) return;
    setCancelarEnvioId(null);
    setError("");
  }
  ...
  const modalCancelarEnvio =
    cancelarEnvioId !== null && typeof document !== "undefined"
      ? createPortal(
          <>
            <button
              type="button"
              aria-label="Cerrar confirmación para cancelar envío"
              className="fixed inset-x-0 top-12 z-[130] bg-slate-950/20 backdrop-blur-[5px]"
              style={{
                bottom: "var(--featmusic-menu-movil-altura, 0px)",
              }}
              onClick={cerrarModalCancelarEnvio}
            />

            <div
              className="pointer-events-none fixed inset-x-0 top-12 z-[135] flex items-center justify-center px-4 py-5"
              style={{
                bottom: "var(--featmusic-menu-movil-altura, 0px)",
              }}
            >
              <section
                role="dialog"
                aria-modal="true"
                aria-labelledby="cancelar-envio-titulo"
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex min-w-0 items-center gap-2.5">
                    <span className="inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl border border-yellow-300 bg-yellow-400 text-black">
                      <IconoRechazar className="h-4.5 w-4.5" />
                    </span>
                ...
                <p className="mt-4 text-[11px] leading-[1.15rem] text-slate-600 sm:text-xs sm:leading-5">
                  El audio enviado se eliminará y podrás enviar una versión
                  nueva después.
                </p>
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  ...
                  <button
                    ...
                    disabled={procesandoId !== null}
                    onClick={() => {
                      if (cancelarEnvioId !== null) {
                        void cancelarPropuesta(cancelarEnvioId);
                      }
                    }}
                  ...
                  >
                    <IconoRechazar className="h-3.5 w-3.5" />
                    {procesandoId !== null ? "Cancelando..." : "Cancelar envío"}
                  </button>
                </div>
              </section>
            </div>
  ...
  return (
    <div className="featmusic-private-profile-sections min-w-0">
      ...
      <style>{`
        ...
        /* FEATMUSIC_MP3_RECIBIDAS_DISENO_ACTIVAS_V1 */
        .featmusic-private-profile-sections .featmusic-private-received-actions .featmusic-received-mp3-button {
          background: #facc15 !important;
          background-color: #facc15 !important;
          color: #000000 !important;
          -webkit-text-fill-color: #000000 !important;
        ...
        .featmusic-private-profile-sections .featmusic-private-received-actions .featmusic-received-mp3-button:hover {
          background: #fde047 !important;
          background-color: #fde047 !important;
          color: #000000 !important;
          -webkit-text-fill-color: #000000 !important;
      ...
      `}</style>
      <div className="flex w-full items-center gap-2 px-1 pb-1 lg:px-0">
        <div className="grid min-w-0 flex-1 grid-cols-3 gap-1.5 sm:flex sm:w-auto sm:flex-none sm:items-center sm:gap-2 sm:overflow-visible">
          <BotonPestana
        ...
        <button
          ...
          disabled={cantidadIdeasActivas >= limiteIdeasActivas}
          aria-label="Abrir el formulario para publicar una idea"
          aria-haspopup="dialog"
          aria-expanded={publicarAbierto}
          className="ml-auto hidden shrink-0 items-center justify-center gap-2 rounded-xl bg-[#FFD400] px-4 py-2 text-[11px] font-black text-black transition hover:bg-[#FFD400] hover:text-black disabled:cursor-not-allowed disabled:opacity-50 lg:inline-flex"
        >
          <IconoIdea className="h-4 w-4" />
          {cantidadIdeasActivas >= limiteIdeasActivas
            ? `Límite ${limiteIdeasActivas}/${limiteIdeasActivas}`
            : "Publicar idea"}
        </button>
      </div>
                              ...
                              <div className="featmusic-private-sent-actions flex h-[30px] bg-white">
                                ...
                                {propuesta.estado === "PENDIENTE" && (
                                  <button
                                    type="button"
                                    disabled={procesando}
                                    onClick={() => abrirModalCancelarEnvio(propuesta.id)}
      ...
      {publicarAbierto && (
        <IdeasMusicalesCard
          ideasIniciales={IDEAS_VACIAS_PUBLICAR}
          limiteIdeasActivas={limiteIdeasActivas}
          cantidadIdeasActivasExterna={cantidadIdeasActivas}
          soloModal
          abiertoExterno
