...
// FEATMUSIC_MODAL_ENVIAR_PROPUESTA_AMARILLO_V1
...
type RespuestaPlanActual = {
  ok?: boolean;
  plan?: string;
};

// FEATMUSIC_PRO_CUPOS_COMPLETOS_V1
// En páginas con muchas ideas puede haber muchos EnviarPropuesta montados.
// Compartimos la consulta para no pedir /api/planes/mi-plan una vez por tarjeta.
let promesaPlanActual: Promise<string | null> | null = null;

function consultarPlanActual() {
  if (!promesaPlanActual) {
    promesaPlanActual = fetch("/api/planes/mi-plan", {
      cache: "no-store",
      credentials: "same-origin",
    })
      .then(async (response) => {
        if (!response.ok) return null;
        const data = (await response.json()) as RespuestaPlanActual;
        return data.ok && typeof data.plan === "string" ? data.plan : null;
      })
      .catch(() => {
        promesaPlanActual = null;
        return null;
      });
  }

  return promesaPlanActual;
}
...
export default function EnviarPropuesta({
  ...
  const [esUsuarioPro, setEsUsuarioPro] = useState(false);
  ...
  useEffect(() => {
    ...
    if (
      !sesionActiva ||
      esPropietario ||
      totalPropuestas < limitePropuestas
    ) {
      setEsUsuarioPro(false);
      return;
    }

    void consultarPlanActual().then((plan) => {
      if (activo) {
        setEsUsuarioPro(plan === "PRO");
      }
    });
  ...
  async function enviarPropuesta(evento: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (!ocupabaCupoAntes) {
        // Un envío PRO puede hacer que el contador supere el límite normal
        // (por ejemplo 4/3), por eso no lo recortamos al máximo de la idea.
        setTotalPropuestas((actual) => actual + 1);
      }
  ...
  const cuposCompletos = totalPropuestas >= limitePropuestas;
  // El servidor vuelve a validar el plan. Esto solo controla la experiencia UI.
  const bloqueadoPorCupos = cuposCompletos && !esUsuarioPro;
  ...
  const puedeReintentar =
    ...
    (propuestaPropia?.numeroIntento ?? 0) < 2;
  const formularioBloqueado = puedeReintentar && bloqueadoPorCupos;
  ...
  const subtituloFormulario = puedeEnviarCorreccion
    ...
    : puedeReintentar
      ? cuposCompletos && esUsuarioPro
        ? "Beneficio Pro: puedes enviar aunque no haya cupos"
        : "El nuevo intento ocupará un cupo disponible"
      : cuposCompletos && esUsuarioPro
        ? "Beneficio Pro: puedes enviar aunque no haya cupos"
        : "Colaborar con esta idea";
  const cuposDisponibles = Math.max(0, limitePropuestas - totalPropuestas);
  ...
  const claseAccion =
    `${claseSegmento} !bg-yellow-400 !text-black transition hover:!bg-yellow-300 disabled:cursor-not-allowed disabled:!bg-slate-100 disabled:!text-slate-400`;
    ...
    return (
      <div className="mt-3 flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
        ...
        <span className="text-[10px] font-bold text-slate-600">
          {totalPropuestas}/{limitePropuestas} cupos ocupados
        </span>
      </div>
    );
  }
      ...
      {variante === "integrada" ? (
            ...
            <p className="text-[9px] leading-4 text-slate-500">
              {puedeEnviarCorreccion
                ...
                : formularioBloqueado
                  ...
                  : cuposCompletos && esUsuarioPro
                    ? "Tu plan Pro te permite participar otra vez aunque la idea esté completa."
        ...
        <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
          <p className="text-[10px] font-medium text-slate-500">
            {cuposCompletos
              ? esUsuarioPro
                ? "Cupos completos. Tu plan Pro te permite enviar igualmente."
                : `Esta idea ya tiene sus ${limitePropuestas} cupos ocupados.`
              : `${limitePropuestas - totalPropuestas} cupo${
                  limitePropuestas - totalPropuestas === 1 ? "" : "s"
                } disponible${
                  limitePropuestas - totalPropuestas === 1 ? "" : "s"
                }.`}
          </p>
          ...
          >
            ...
            >
              <div className="flex shrink-0 items-start justify-between gap-4 border-b border-slate-200 bg-white/95 px-4 py-3.5 backdrop-blur lg:px-5 lg:py-4">
                <div className="min-w-0">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-yellow-500 lg:text-xs lg:normal-case lg:tracking-normal">
                    {subtituloFormulario}
                  </p>
                  <h3
              ...
              >
                <div>
                  ...
                  <input
                    ...
                    disabled={enviando}
                    className="mt-2 block w-full rounded-xl border border-slate-200 bg-white px-3 py-3 text-xs text-slate-700 file:mr-3 file:rounded-lg file:border-0 file:bg-yellow-400 file:px-3 file:py-2 file:text-[10px] file:font-bold file:text-black"
                  />
                ...
                <div className="mt-4">
                  ...
                  <textarea
                    ...
                    className="mt-2 w-full scroll-mt-4 scroll-mb-6 resize-none rounded-xl border border-slate-200 bg-white px-3 py-3 text-xs text-slate-800 outline-none transition placeholder:text-slate-400 focus:border-yellow-400 focus:ring-2 focus:ring-yellow-100"
                  />
                </div>
              ...
              <div className="grid shrink-0 grid-cols-2 gap-2 border-t border-slate-200 bg-white/95 px-4 py-3 backdrop-blur lg:flex lg:justify-end lg:px-5 lg:pb-4 lg:pt-3">
                ...
                <button
                  ...
                  className="min-h-11 rounded-xl border border-yellow-400 bg-yellow-400 px-3 py-2.5 text-xs font-black text-black transition hover:border-yellow-300 hover:bg-yellow-300 disabled:cursor-not-allowed disabled:opacity-45 lg:px-4"
                >
