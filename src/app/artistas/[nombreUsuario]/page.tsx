...
import CentroNotificaciones from "../../panel/components/CentroNotificaciones";
...
function InsigniaPlan({
  ...
  const etiqueta = esPro ? "Plan Pro" : "Plan Creator";
  ...
  return (
    ...
    >
      ...
      >
        <path
          d="M12 2.35 14.05 4l2.62-.25.88 2.48 2.32 1.25-.35 2.62L21 12l-1.48 1.9.35 2.62-2.32 1.25-.88 2.48-2.62-.25L12 21.65 9.95 20l-2.62.25-.88-2.48-2.32-1.25.35-2.62L3 12l1.48-1.9-.35-2.62 2.32-1.25.88-2.48L9.95 4 12 2.35Z"
          className={
            esPro
              ? "fill-[#E2B93B] stroke-[#FFE69A]"
              : "fill-[#AEB6C2] stroke-[#EEF2F7]"
          }
          strokeWidth="0.9"
          strokeLinejoin="round"
        />
        <path
          d="m8.3 12.1 2.3 2.25 5.1-5.15"
          stroke={esPro ? "#17120A" : "#111827"}
          strokeWidth="2.05"
          strokeLinecap="round"
          strokeLinejoin="round"
        />
...
export default async function PerfilPublicoPage({
  ...
  const artistasProSugeridos = artistasProDisponibles
    .map((artistaPro) => ({
      artistaPro,
      ordenAleatorio: Math.random(),
    }))
    .sort((a, b) => a.ordenAleatorio - b.ordenAleatorio)
    .slice(0, 10)
    .map(({ artistaPro }) => artistaPro);
  ...
  const tarjetasIdeasActivas = (
    <>
          ...
          {artista.ideas.map((idea, indice) => {
            ...
            return (
              <article
                ...
                key={idea.id}
                data-vista-idea
                className="relative scroll-mt-16 overflow-visible rounded-xl border border-slate-200 bg-white transition hover:shadow-[0_10px_28px_rgba(15,23,42,0.07)]"
              >
                <RegistrarVistaIdea
                  ideaId={idea.id}
                ...
                <div className="grid grid-cols-3 divide-x divide-slate-200 overflow-hidden rounded-b-xl border-t border-slate-200 bg-white">
                  ...
                  <EnviarPropuesta
                    ...
                    limitePropuestas={limitesPlan.propuestasPorIdea}
      ...
      {artistasProSugeridos.length > 0 && (
        <div className="mt-6 sm:mt-7">
          <div className="mb-3 px-1">
            <p className="text-[10px] font-semibold text-slate-500 sm:text-[11px]">
              Artistas que podrían interesarte:
            </p>
          </div>

          <div className="-mx-1 flex snap-x snap-mandatory gap-4 overflow-x-auto px-1 pb-2 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden sm:gap-5">
            {artistasProSugeridos.map((artistaPro) => {
              const nombreArtistaPro =
                artistaPro.nombreArtistico?.trim() ||
                artistaPro.nombre?.trim() ||
                artistaPro.nombreUsuario?.trim() ||
                "Artista";
              const usuarioArtistaPro =
                artistaPro.nombreUsuario?.trim() || `artista-${artistaPro.id}`;
              const codigoPaisArtistaPro = resolverCodigoPais(
                artistaPro.pais ?? "",
              );
              ...
              return (
                <Link
                  key={artistaPro.id}
                  href={`/artistas/${encodeURIComponent(usuarioArtistaPro)}`}
                  className="group flex w-[92px] shrink-0 snap-start flex-col items-center text-center sm:w-[104px]"
                >
                  {artistaPro.fotoPerfil ? (
                    <img
                      src={artistaPro.fotoPerfil}
                      alt={`Foto de ${nombreArtistaPro}`}
                      className="h-16 w-16 rounded-full border border-slate-200 object-cover shadow-sm transition duration-200 group-hover:scale-[1.04] dark:border-white/20 sm:h-[72px] sm:w-[72px]"
                      loading="lazy"
                    />
                  ) : (
                    <span className="flex h-16 w-16 items-center justify-center rounded-full border border-yellow-400 bg-yellow-400 text-sm font-black text-black shadow-sm transition duration-200 group-hover:scale-[1.04] sm:h-[72px] sm:w-[72px]">
                      {iniciales(nombreArtistaPro)}
                    </span>
                  )}

                  <span className="mt-2 flex w-full min-w-0 items-center justify-center gap-1">
                    <span className="min-w-0 truncate text-[10px] font-black text-slate-900 dark:text-white sm:text-[11px]">
                      {nombreArtistaPro}
                    </span>

                    {codigoPaisArtistaPro && (
                      <img
                        src={`https://flagcdn.com/w40/${codigoPaisArtistaPro.toLowerCase()}.png`}
                        srcSet={`https://flagcdn.com/w80/${codigoPaisArtistaPro.toLowerCase()}.png 2x`}
                        width={20}
                        height={14}
                        alt={`Bandera de ${artistaPro.pais ?? ""}`}
                        title={artistaPro.pais ?? ""}
                  ...
                  <span className="mt-0.5 block w-full truncate text-[9px] font-semibold text-slate-500 dark:text-slate-400 sm:text-[10px]">
                    @{usuarioArtistaPro}
                  </span>
                </Link>
              );
      ...
      >
        <div className="grid gap-3 sm:gap-4 lg:grid-cols-[310px_minmax(0,1fr)]">
          ...
          >
            ...
            <div className="relative z-10 flex items-start gap-3.5 lg:flex-col lg:items-center lg:text-center">
              ...
              <div className="min-w-0 flex-1 pt-0.5 lg:w-full lg:px-10 lg:pt-0">
                <div className="pr-28 sm:pr-32 lg:pr-0">
                  <div className="flex min-w-0 items-center gap-1.5 lg:justify-center lg:pt-4">
                    <h1
                      className={`min-w-0 break-words text-xl font-black leading-tight sm:text-2xl ${
                        artista.portadaPerfil
                    ...
                    <InsigniaPlan
                      plan={planArtista}
                      className="h-[19px] w-[19px] sm:h-5 sm:w-5"
                    />
                  </div>
                  <p className="mt-1 break-all text-[11px] font-semibold text-[#FFD400] sm:text-xs">
                    @{usuarioVisible}
                  </p>
            ...
            {esPerfilPropio && (
              <div className="featmusic-profile-dark-controls relative z-10 -mx-4 -mb-4 mt-4 grid grid-cols-2 divide-x divide-slate-200 border-t border-slate-200 bg-white sm:-mx-5 sm:-mb-5">
                <PerfilArtistaCard
                  modo="controles"
          ...
          >
            {esPerfilPropio ? (
              <SeccionesPerfilPrivado
                ...
                limiteIdeasActivas={limitesPlan.ideasActivas}
